...
*/
  public function __construct($name, $attributes=array()) {
    ...
    $this['name'] = $name;
    if(is_callable('CChris::Instance()')) {
      $this->characterEncoding = CChris::Instance()->config['character_encoding'];
    } else {
...
*/
  public function GetHTML() {
    ...
    $readonChris = isset($this['readonChris']) && $this['readonChris'] ? " readonChris='readonChris'" : null;
    $type = isset($this['type']) ? " type='{$this['type']}'" : null;
    $onChrisValue = isset($this['value']) ? htmlentities($this['value'], ENT_COMPAT, $this->characterEncoding) : null;
    $value = isset($this['value']) ? " value='{$onChrisValue}'" : null;
    ...
    if($type && $this['type'] == 'submit') {
      return "<span><input id='$id'{$type}{$class}{$name}{$value}{$autofocus}{$readonChris} /></span>\n";
    } else if($type && $this['type'] == 'textarea') {
      return "<p><label for='$id'>$label</label><br><textarea id='$id'{$type}{$class}{$name}{$autofocus}{$readonChris}>{$onChrisValue}</textarea></p>\n";
    } else if($type && $this['type'] == 'hidden') {
      return "<input id='$id'{$type}{$class}{$name}{$value} />\n";
    } else {
      return "<p><label for='$id'>$label</label><br><input id='$id'{$type}{$class}{$name}{$value}{$autofocus}{$readonChris} />{$messages}</p>\n";	
    }
  }
