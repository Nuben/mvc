...
// include the site specific config.php and create a ref to $Chris to be used by config.php
$Chris = &$this;
...
*/
  ...
  /**
* ThemeEngineRender, renders the repChris of the request to HTML or whatever.
*/
  public function ThemeEngineRender() {
    ...
    $themePath = Chris_INSTALL_PATH . '/' . $this->config['theme']['path'];
    ...
    if(isset($this->config['theme']['parent'])) {
      $parentPath = Chris_INSTALL_PATH . '/' . $this->config['theme']['parent'];
      $parentUrl	= $this->request->base_url . $this->config['theme']['parent'];
    }
    
    // Add stylesheet name to the $Chris->data array
    $this->data['stylesheet'] = $this->config['theme']['stylesheet'];
    
    // Make the theme urls available as part of $Chris
    ...
    $Chris = &$this;
    // First the default Chris themes/functions.php
    ...
    // Extract $Chris->data to own variables and handover to the template file
...
public function CreateUrl($urlOrController=null, $method=null, $arguments=null) {
  ...
  /**
* Draw HTML for a menu defined in $Chris->config['menus'].
*
