    ...
    switch($action) {
      case 'install':
        try {
          ...
          $this->db->ExecuteQuery(self::SQL('insert content'), array('home', 'page', 'Home page', "This is a demo page, this could be your personal home-page.\n\nChris is a PHP-based MVC-inspired Content management Framework, watch the making of Chris at: http://dbwebb.se/Chrisdia/tutorial.", 'plain', $this->user['id']));
          $this->db->ExecuteQuery(self::SQL('insert content'), array('about', 'page', 'About page', "This is a demo page, this could be your personal about-page.\n\nChris is used as a tool to educate in MVC frameworks.", 'plain', $this->user['id']));
          $this->db->ExecuteQuery(self::SQL('insert content'), array('download', 'page', 'Download page', "This is a demo page, this could be your personal download-page.\n\nYou can download your own copy of Chrisdia from https://github.com/mosbth/Chrisdia.", 'plain', $this->user['id']));
          $this->db->ExecuteQuery(self::SQL('insert content'), array('bbcode', 'page', 'Page with BBCode', "This is a demo page with some BBCode-formatting.\n\n[b]Text in bold[/b] and [i]text in italic[/i] and [url=http://dbwebb.se]a link to dbwebb.se[/url]. You can also include images using bbcode, such as the Chrisdia logo: [img]http://dbwebb.se/Chrisdia/current/themes/core/logo_80x80.png[/img]", 'bbcode', $this->user['id']));
          ...
          return array('success', 'SuccessfulChris created the database tables and created a default "Hello World" blog post, owned by you.');
        } catch(Exception$e) {
...
*/
  public function Save() {
    ...
    if($rowcount) {
      $this->AddMessage('success', "SuccessfulChris {$msg} content '" . htmlEnt($this['key']) . "'.");
    } else {
...
*/
  public function Delete() {
    ...
    $rowcount = $this->db->RowCount();
    if($rowcount) {
      $this->AddMessage('success', "SuccessfulChris set content '" . htmlEnt($this['key']) . "' as deleted.");
    } else {
